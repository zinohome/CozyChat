...
const log = logger.withTag('ChatPage');
...
export const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (preferences?.default_personality && personalityId === 'default') {
      log.debug('使用用户偏好设置的人格', preferences.default_personality);
      ...
      const loadDefaultPersonality = async () => {
        try {
          ...
          if (personalities.length > 0) {
            ...
            log.debug('从人格列表设置默认人格', selectedPersonalityId);
            setPersonalityId(selectedPersonalityId);
          } else {
            log.debug('人格列表为空，保持默认人格 default');
          }
        } catch (error) {
          log.error('加载人格列表失败', error);
    ...
    const autoSelectSession = async () => {
      try {
        if (sessions.length > 0) {
          ...
          log.debug('选择第一个会话', firstSessionId);
          if (firstSessionId) {
          ...
          log.debug('没有会话，创建新会话，使用人格:', sessionPersonalityId, {
          ...
          log.debug('新会话创建成功', newSessionId);
          if (newSessionId) {
