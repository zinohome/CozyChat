...
当前使用 OpenAI Agents SDK 的 `OpenAIRealtimeWebRTC` 实现实时语音通话。

**核心文件**：
- `hooks/useVoiceAgent.ts` - 语音通话 Hook（816行）
- `features/chat/components/EnhancedChatContainer.tsx` - 集成语音通话
- `features/chat/components/VoiceCallIndicator.tsx` - 语音通话指示器
...
为了支持多种传输方式（WebRTC、WebSocket），计划进行架构重构。

**详细方案**：参见 `docs/architecture/frontend-voice-agent-refactoring.md`

**重构目标**：
1. 分离传输层、会话管理、音频可视化等模块
2. 支持通过配置选择传输方式
3. 提高代码可维护性和可测试性
