...
> **版本**: v2.0  
> **最后更新**: 2025-11-18  
> **变更**: 新增session_contexts表，优化sessions表
...
### v2.0 (2025-11-18)
- ✅ 新增`session_contexts`表 - 存储历史摘要
- ✅ `sessions`表新增`title_generated_at`字段
- ✅ 添加复合索引优化查询性能
- ✅ 向量数据库新增`mixed_memories` collection
...
CREATE TABLE sessions (
    ...
    title_generated_at TIMESTAMP, -- v2.0新增：标题生成时间
...
CREATE INDEX idx_sessions_user_created ON sessions(user_id, created_at DESC); -- v2.0新增
```
...
#### session_contexts（会话上下文表） - v2.0新增
...
CREATE TABLE session_contexts (
    ...
    -- 上下文类型
    context_type VARCHAR(50) NOT NULL, -- 'history_summary' / 'memory_snapshot' / 'topic_summary'
    ...
    CONSTRAINT check_context_type CHECK (context_type IN ('history_summary', 'memory_snapshot', 'topic_summary'))
);
...
-- 全文搜索索引
CREATE INDEX idx_session_contexts_content_search ON session_contexts USING gin(
    to_tsvector('simple', content)
);
```
